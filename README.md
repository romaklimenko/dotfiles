...
- **Claude Code Configuration:** Shared [Claude Code](https://claude.ai/) settings across machines
  ...
  - Session notes: a hook writes lessons from every session to `LESSONS.md` files at project, workspace and user level, compacts them when they grow, and reads them back at session start. `/lessons` shows where they are
